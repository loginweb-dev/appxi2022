...
<script>
    ...
    async function get_list() {
        ...
        //console.log(chofer.data.ciudad.id)
        ...
        if(consulta_viaje_disponible.data){
            ...
            var milist= await axios.get("{{setting('admin.url_api')}}viajes_chofer/"+chofer.data.ciudad.id)
            var miul = ''
            for (let index = 0; index < milist.data.length; index++) {
                ...
                miul = miul + "<li class='collection-item avatar'><img src='{{ setting('admin.url_storage') }}"+img+"' class='circle'><span class='title'>"+milist.data[index].cliente.nombres+' '+milist.data[index].cliente.apellidos+"</span><p>Fecha y Hora: "+milist.data[index].published+"</p><p>Precio Ofertado: "+milist.data[index].precio_inicial+" Bs.</p><p>Distancia: "+(milist.data[index].distancia).toFixed(2)+" Km</p><p>Tiempo: "+(milist.data[index].tiempo).toFixed(2)+" Minutos</p><a class='secondary-content tooltipped' data-position='bottom' data-tooltip='Aceptar Viaje' onclick='aceptar_viaje("+milist.data[index].id+")'><i class='material-icons'>send</i></a></li>"
